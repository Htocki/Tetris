...
begin
  ...
  matrix[6, 3] := 3;
  result := result and
            (figAddOTop(matrix, position) = false) and
  ...
  position.y := 4;
  result := result and
            (figAddOTop(matrix, position) = true) and
            ...
            (matrix[3, 10] = 4);
  tesMessage('figure', 'add_o_top', result);
end.
